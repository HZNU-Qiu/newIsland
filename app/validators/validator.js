...
class AssemblePaperValidator extends PositiveIntegerValidator {
  ...
  async validateScore(vals) {
    ...
    if (exam) {
      throw new Error('有考试正在进行，不能修改试卷')
    }
    const score1 = parseInt(vals.body.score)
...
class StartToExamValidator extends PositiveIntegerValidator {
  ...
  async validateExamIsStart(vals) {
    ...
    if (parseInt(userExam.status) === 1) {
      throw new Error('您已提交考试')
    }
  }
}
 ...
 * 试卷启用校验器
 */
class ActivatePaperValidator extends PositiveIntegerValidator {
  constructor() {
    super()
  }
  async validateHasExercises(vals) {
    const id = vals.path.id
    const paper = await Paper.findByPk(id)
    if (!paper.problem_list) {
      throw new Error('试卷内无题目，不能发布')
    }
  }
}
 ...
 * 用户提交试卷校验器
 */
class SubmitValidator extends PositiveIntegerValidator {
  constructor() {
    super()
  }
  async validateAccessToSubmit(vals) {
    const exam_id = vals.path.id
    const exam = await Exam.findByPk(exam_id)
    if (parseInt(exam.status) !== 1) {
      throw new Error('考试已经结束不能提交')
    }
  }
}
